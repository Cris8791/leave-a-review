document.addEventListener("DOMContentLoaded", () => {
  ...
  phoneNumberInput.addEventListener("change", () => {
    const numberWithoutPrefix = iti.getNumber(intlTelInputUtils.numberFormat.NATIONAL);

    if (numberWithoutPrefix.startsWith(`+${iti.getSelectedCountryData().dialCode}`)) {
      phoneNumberInput.value = numberWithoutPrefix.slice(iti.getSelectedCountryData().dialCode.length + 1);
    } else {
